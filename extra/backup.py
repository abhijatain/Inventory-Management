...
def display_aggrid(df):
    ...
    gb.configure_side_bar()  # Add a sidebar (includes search functionality)
    gb.configure_default_column(editable=True)  # Allow columns to be editable

    grid_options = gb.build()

    # Display grid with AgGrid
    AgGrid(
        df, 
        gridOptions=grid_options,
        enable_enterprise_modules=True,  # Enables advanced features like search
        allow_unsafe_jscode=True,  # Enables code customization if needed
    )
...
with st.sidebar:
    selected = option_menu(
        menu_title = "Menu",
        options = ["Inventory Analysis","Customer Analysis","Advanced Analysis","Guide","Contact Us"],
        icons = ["house","gear","activity","book","envelope"],
        menu_icon = "cast",
        default_index = 0,
        #orientation = "horizontal",
    )
if selected == "Inventory Analysis":
    ...
    with st.spinner('Updating Report...'):
        if uploaded_file is not None:
            ...
            all_widgets = sp.create_widgets(understock, create_data)
            res = sp.filter_df(understock, all_widgets)

            st.title('Re-order')
            csv = convert_df(understock)
            st.download_button(
                "Press to Download",
                csv,
                "reorder.csv",
                "text/csv",
                key='download-csv-reorder'
                )
            st.dataframe(res, use_container_width=True)
            
            st.title('Overstocked')
            display_aggrid(overstock)
...
if selected == "Customer Analysis":
    ...
    uploaded_file_2 = st.file_uploader("Upload an Excel file", type=["xls", "xlsx"])
