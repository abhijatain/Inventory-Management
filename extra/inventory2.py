...
with col1:
    lead_time = st.number_input("Enter Lead Time (in days)", min_value=1, value=5,help='Time Taken for Items to be delivered')

# Place input widgets in the second column
with col2:
    days_stock_to_maintain = st.number_input("Enter Days of Stock to Maintain", min_value=1, value=30,help='Enter Days for which you wish to maintain stock')

# Place input widgets in the third column
with col3:
    date_range = st.date_input("Select Date Range for Data", [datetime.today(), datetime.today()],help='Enter the time period for which you have the data')
...
with st.spinner('Updating Report...'):
    if uploaded_file is not None:
        ...
        voucher_types = ['All'] + list(voucher_types)  
        selected_voucher = st.selectbox('Choose Voucher Type', voucher_types, help='Filter report to show specific voucher types')

        # If 'All' is selected, show all data, otherwise filter based on the voucher type
        if selected_voucher == 'All':
            filtered_df = cleaned_df
        else:
            filtered_df = cleaned_df[cleaned_df['Voucher Type'] == selected_voucher]
