...
from components.data_processing3 import clean_and_process_data
from components.user_inputs import get_user_inputs
from components.display_metrics import display_metrics, display_charts
from components.calculations import calculate_overstock, calculate_understock, calculate_negative_stock, calculate_unsold_stock


st.set_page_config(page_title='Godown Inventory Analysis',  layout='wide', page_icon=':ambulance:')

st.title("Godown Inventory Analysis")
...
with st.spinner('Updating Report...'):
    ...
    # Clean and process data
        ...
        st.markdown(
            """
            #### Go to Godown and configure the following settings:
            """
        )

        st.image("image.png", width=420)
